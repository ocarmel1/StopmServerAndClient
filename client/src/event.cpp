...
Event::Event(std::string sender, std::string team_a_name, std::string team_b_name, std::string name, int time,
             std::map<std::string, std::string> game_updates, std::map<std::string, std::string> team_a_updates,
             std::map<std::string, std::string> team_b_updates, std::string discription)
    : sender(sender), team_a_name(team_a_name), team_b_name(team_b_name), name(name),
...
const std::string &Event::get_sender() const
{
    return this->sender;
}
...
Event::Event(const std::string &frame_body) :sender(""), team_a_name(""), team_b_name(""), name(""), time(0), game_updates(), team_a_updates(), team_b_updates(), description("")
{
}

names_and_events parseEventsFile(std::string json_path, std::string sender)
{
    ...
    for (auto &event : data["events"])
    {
        ...
        events.push_back(Event(sender,team_a_name, team_b_name, name, time, game_updates, team_a_updates, team_b_updates, description));
    }
